...
class Message {
    ...
    class func send(message: Message, toID: String, completion: @escaping (Bool) -> ())  {
        if let currentUserID = Auth.auth().currentUser?.uid {
            switch message.type {
            case .location:
    ...
    class func uploadMessage(withValues: [String: Any], toID: String, completion: @escaping (Bool) -> ()) {
        if let currentUserID = Auth.auth().currentUser?.uid {
